...
export default class Statistic extends Event {
  ...
  _init() {
    ...
    browserEvent.create(window, 'unload', () => {
      storage.setData(config.namespaces + '_pageId_' + config.appId, config.pageId, true);
      this.$emit('windowUnload', Object.assign(behavior.getBehavior(), activeTime.getActiveTime()));
