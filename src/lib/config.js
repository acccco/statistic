...
config.sessionId = storage.getData(config.namespaces + "_sessionId_" + config.appId) || Math.round(new Date() / 1E3);
storage.setData(config.namespaces + "_sessionId_" + config.appId, config.sessionId, true);

// 页面唯一 ID 生成，根据 appId 和当前 url
config.pageId = md5(config.appId + window.location.href);

// 获取父页面的 ID
let parentPageId = storage.getData(config.namespaces + "_pageId_" + config.appId);
