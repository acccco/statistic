...
export function getPageInfo() {
  let data = {
    ...
    ppai: config.parentPageAccessId,
    tt: document.title,
    ct: 1,
    pvc: 0
  }
  let history = storage.getData(config.namespaces + "_pvet_" + config.appid) || 0
  data.ct = getPageInType(history)

  let historyList = storage.getData(config.namespaces + "_pvlt_" + config.appid)
  historyList = historyList ? historyList.split(',') : []
  data.pvc = historyList.length
  if (historyList[historyList.length - 1] !== config.pageAccessId) {
    historyList.push(config.pageAccessId)
  }
