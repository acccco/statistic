...
export default function getPageInfo() {
  ...
  let lastInTime = storage.getData(config.namespaces + "_pvlt_" + config.appId) || 0
  data.channelType = getPageInType(lastInTime)

  let historyList = storage.getData(config.namespaces + "_pvhl_" + config.appId)
  ...
  storage.setData(config.namespaces + "_pvhl_" + config.appId, historyList.join(','), true)
  storage.setData(config.namespaces + "_pvlt_" + config.appId, config.pvStartTime, true)
 ...
 * @returns {boolean}
 */
function includeUrl(allUrl, domain) {
  allUrl = allUrl.replace(/^https?:\/\//, "");
  return 0 === allUrl.indexOf(domain)
}
